...
console.log("Conectando a Read en:", process.env.DB_HOST_READ);
...
const UserInRead = UserModel(readDB);
...
async function syncTables() {
    try {
        ...
        for (const user of usersInCreate) {
            const existingUser = await UserInRead.findByPk(user.id);
            if (!existingUser) {
                await UserInRead.create(user.toJSON());
